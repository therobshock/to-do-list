...
function shitAnalysis() {
  var byUrgency = shitList.sort((a, b) => a.urgency - b.urgency);
  var byEffect = shitList.sort((a, b) => a.effect - b.effect);
  var byDanger = shitList.sort((a, b) => a.danger - b.danger);
  var byCost = shitList.sort((a, b) => a.cost - b.cost);
  var byHealth = shitList.sort((a, b) => a.health - b.health);
  var byRating = shitList.sort((a, b) => a.rating - b.rating);
 
  const listDiv = $("<div>");
  const mainList = $("<ol>");
  const urgencyList = $("<ol>");
  const effectList = $("<ol>");
  const dangerList = $("<ol>");
  const costList = $("<ol>");
  const healthList = $("<ol>");
  ...
  for (var i = 0; i < 3; i++) {
    var listItem = $("<li>");
    listItem.append(byRating[i].name);
